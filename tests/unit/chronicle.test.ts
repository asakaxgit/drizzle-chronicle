...
type VersionedTestRecord = VersionedRecord<TestRecord>;
...
describe("Chronicle", () => {
	...
	describe("Table Registration", () => {
		...
		test("should create history table when autoCreate is enabled", () => {
			...
			const testSqlite = (db as unknown as { session: { client: Database.Database } }).session
				.client;
	...
	describe("Insert Operations", () => {
		test("should insert record with versioning", async () => {
			...
			const history = sqlite
				.prepare(`SELECT * FROM test_table_history WHERE id = 1`)
				.get() as VersionedTestRecord;
			expect(history).toBeDefined();
			expect(history.version_operation).toBe("INSERT");
		});
	...
	describe("Update Operations", () => {
		test("should update record and create version", async () => {
			...
			const record = sqlite.prepare(`SELECT * FROM test_table WHERE id = 1`).get() as TestRecord;
			expect(record.name).toBe("Updated");

			// Verify version history
			const versions = await chronicle.getVersions<VersionedTestRecord>("test_table", { id: 1 });
	...
	describe("Delete Operations", () => {
		test("should delete record and record in history", async () => {
			...
			const versions = await chronicle.getVersions<VersionedTestRecord>("test_table", { id: 1 });
			const deleteVersion = versions[versions.length - 1];
	...
	describe("Version Queries", () => {
		...
		test("should retrieve specific version by ID", async () => {
			...
			const version = await chronicle.getVersion<VersionedTestRecord>("test_table", versionId);

			expect(version).toBeDefined();
			expect(version?.name).toBe("Test");
		});
	...
	describe("Rollback", () => {
		test("should rollback to previous version", async () => {
			...
			const versions = await chronicle.getVersions<VersionedTestRecord>("test_table", { id: 1 });
			const version2Id = versions[1].version_id;
			...
			const current = sqlite.prepare(`SELECT * FROM test_table WHERE id = 1`).get() as TestRecord;
