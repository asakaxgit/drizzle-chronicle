...
describe("Integration: Complete Versioning Workflows", () => {
	test("should track complete lifecycle of a record", async () => {
      ...
      `)
			...
			expect(versions[0].version_operation).toBe("INSERT");
			expect(versions[1].version_operation).toBe("UPDATE");
			expect(versions[2].version_operation).toBe("UPDATE");
			expect(versions[3].version_operation).toBe("UPDATE");
			expect(versions[4].version_operation).toBe("DELETE");

			// Verify data at each version
			expect(versions[0].name).toBe("John Doe");
			expect(versions[0].role).toBe("user");
			expect(versions[1].role).toBe("admin");
			expect(versions[2].email).toBe("john.doe@example.com");
			expect(versions[3].name).toBe("John Smith");
		});
	});
	...
	test("should handle multiple records independently", async () => {
      ...
      `)
			...
			const aliceVersions = await chronicle.getVersions<VersionedUser>("users", { id: 1 });
			const bobVersions = await chronicle.getVersions<VersionedUser>("users", { id: 2 });

			expect(aliceVersions).toHaveLength(2);
			expect(bobVersions).toHaveLength(2);

			expect(aliceVersions[1].role).toBe("superadmin");
			expect(bobVersions[1].email).toBe("robert@example.com");
		});
	});
	...
	test("should support rollback workflows", async () => {
      ...
      `)
			...
			const versions = await chronicle.getVersions<VersionedUser>("users", { id: 1 });
			const adminVersionId = versions[1].version_id;
			...
			const current = sqlite.prepare(`SELECT * FROM users WHERE id = 1`).get() as User;
	...
	test("should maintain data integrity across operations", async () => {
      ...
      `)
			...
			for (let i = 1; i <= 10; i++) {
				const versions = await chronicle.getVersions<VersionedUser>("users", { id: i });

				if (i <= 5) {
					// Should have INSERT, UPDATE, DELETE
					expect(versions).toHaveLength(3);
					expect(versions[2].version_operation).toBe("DELETE");
				} else {
	...
	test("should handle edge cases correctly", async () => {
      ...
      `)
			...
			const updatedVersions = await chronicle.getVersions<VersionedUser>("users", { id: 1 });
			expect(updatedVersions).toHaveLength(5);

			// Verify all role changes are tracked
			expect(updatedVersions[1].role).toBe("admin");
			expect(updatedVersions[2].role).toBe("user");
			expect(updatedVersions[3].role).toBe("admin");
			expect(updatedVersions[4].role).toBe("superadmin");
		});
	});
});
